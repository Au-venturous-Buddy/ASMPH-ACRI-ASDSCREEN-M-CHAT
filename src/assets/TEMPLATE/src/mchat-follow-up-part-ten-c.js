...
export default function MCHATFollowUpPartTenC(props) {
  const getResult = (scores, state, items) => { 
    generateAnswerSummary(items, state, scores, 9, props.updateFollowUpAnswerSummary);
  
    props.updateFollowUpScoreMatrix(scores[0], 9)
    props.changePart(getNextFollowUpItem(props.followUpItems, state, props));
  }

  return(
    <Questionnaire prevSectionScores={props.scores} partID="10-2" questionFields={questionFields} getResult={getResult} currentLanguage={props.state.currentLanguage} stateMain={props.stateMain} data={props.data} restartTestMain={props.restartTestMain} />
  )
}