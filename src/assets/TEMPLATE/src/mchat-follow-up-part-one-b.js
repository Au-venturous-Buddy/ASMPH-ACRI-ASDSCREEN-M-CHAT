...
import MCHATFollowUpPartOneC from "./mchat-follow-up-part-one-c";
...
export default function MCHATFollowUpPartOneB(props) {
  const getResult = (scores, state, items) => { 
    ...
    if(hasPassAnswers && (!hasFailAnswers)) {
      props.updateFollowUpScoreMatrix(0, 0)
      props.changePart(getNextFollowUpItem(props.followUpItems, state, props))
    }
    else if((!hasPassAnswers) && hasFailAnswers) {
      props.updateFollowUpScoreMatrix(1, 0)
      props.changePart(getNextFollowUpItem(props.followUpItems, state, props))
    }
    else if(hasPassAnswers && hasFailAnswers) {
      props.changePart(<MCHATFollowUpPartOneC state={state} followUpItems={props.followUpItems} patientInfo={props.patientInfo} restartTestMain={props.restartTestMain} scores={scores} data={props.data} stateMain={props.stateMain} changePart={props.changePart} updateFollowUpScoreMatrix={props.updateFollowUpScoreMatrix} updateFollowUpAnswerSummary={props.updateFollowUpAnswerSummary} />)
    }
  }

  return(
    <Questionnaire partID="01-1" questionFields={questionFields} getResult={getResult} currentLanguage={props.state.currentLanguage} stateMain={props.stateMain} data={props.data} restartTestMain={props.restartTestMain} />
  )
}