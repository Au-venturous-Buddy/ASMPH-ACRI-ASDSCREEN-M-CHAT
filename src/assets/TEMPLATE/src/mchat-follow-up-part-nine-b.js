...
import MCHATFollowUpPartNineC from "./mchat-follow-up-part-nine-c";
...
export default function MCHATFollowUpPartNineB(props) {
  const getResult = (scores, state, items) => { 
    ...
    generateAnswerSummary(items, state, scores, 8, props.updateFollowUpAnswerSummary);
  
    if(hasPassAnswers) {
      props.changePart(<MCHATFollowUpPartNineC state={state} followUpItems={props.followUpItems} patientInfo={props.patientInfo} restartTestMain={props.restartTestMain} scores={scores} data={props.data} stateMain={props.stateMain} changePart={props.changePart} changeLanguage={props.changeLanguage} updateFollowUpScoreMatrix={props.updateFollowUpScoreMatrix} updateFollowUpAnswerSummary={props.updateFollowUpAnswerSummary} />)
    }
    else {
      props.updateFollowUpScoreMatrix(1, 8)
      props.changePart(getNextFollowUpItem(props.followUpItems, state, props))
    }
  }

  return(
    <Questionnaire partID="09-1" questionFields={questionFields} getResult={getResult} currentLanguage={props.state.currentLanguage} stateMain={props.stateMain} data={props.data} restartTestMain={props.restartTestMain} />
  )
}