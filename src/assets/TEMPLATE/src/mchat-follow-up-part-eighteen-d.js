...
export default function MCHATFollowUpPartEighteenD(props) {
  const getResult = (scores, state, items) => { 
    ...
    if(hasPassAnswers) {
      props.updateFollowUpScoreMatrix(0, 17)
    }
    else {
      props.updateFollowUpScoreMatrix(1, 17)
    }
    props.changePart(getNextFollowUpItem(props.followUpItems, state, props))
  }

  return(
    <Questionnaire partID="18-3" questionFields={questionFields} getResult={getResult} currentLanguage={props.state.currentLanguage} stateMain={props.stateMain} data={props.data} restartTestMain={props.restartTestMain} />
  )
}