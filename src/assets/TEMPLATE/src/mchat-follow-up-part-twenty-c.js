...
export default function MCHATFollowUpPartTwentyC(props) {
  const getResult = (scores, state, items) => { 
    ...
    if(hasPassAnswers) {
      props.updateFollowUpScoreMatrix(0, 19)
    }
    else {
      props.updateFollowUpScoreMatrix(1, 19)
    }
    props.changePart(getNextFollowUpItem(props.followUpItems, state, props))
  }

  return(
    <Questionnaire partID="20-2" questionFields={questionFields} getResult={getResult} currentLanguage={props.state.currentLanguage} stateMain={props.stateMain} data={props.data} restartTestMain={props.restartTestMain} />
  )
}