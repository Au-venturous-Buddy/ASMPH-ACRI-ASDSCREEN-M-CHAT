...
export default function MCHATFollowUpPartThreeB(props) {
  const getResult = (scores, state, items) => { 
    ...
    if(hasPassAnswers) {
      props.updateFollowUpScoreMatrix(0, 2)
    }
    else {
      props.updateFollowUpScoreMatrix(1, 2)
    }
    props.changePart(getNextFollowUpItem(props.followUpItems, state, props))
  }

  return(
    <Questionnaire partID="03-1" questionFields={questionFields} getResult={getResult} currentLanguage={props.state.currentLanguage} stateMain={props.stateMain} data={props.data} restartTestMain={props.restartTestMainMain} />
  )
}