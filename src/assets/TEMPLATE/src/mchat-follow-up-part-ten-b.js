...
import MCHATFollowUpPartTenC from "./mchat-follow-up-part-ten-c";
...
export default function MCHATFollowUpPartTenB(props) {
  const getResult = (scores, state, items) => { 
    ...
    if(hasPassAnswers && (!hasFailAnswers)) {
      props.updateFollowUpScoreMatrix(0, 9)
      props.changePart(getNextFollowUpItem(props.followUpItems, state, props))
    }
    else if((!hasPassAnswers) && hasFailAnswers) {
      props.updateFollowUpScoreMatrix(1, 9)
      props.changePart(getNextFollowUpItem(props.followUpItems, state, props))
    }
    else if(hasPassAnswers && hasFailAnswers) {
      props.changePart(<MCHATFollowUpPartTenC state={state} followUpItems={props.followUpItems} patientInfo={props.patientInfo} restartTestMain={props.restartTestMain} scores={scores} data={props.data} stateMain={props.stateMain} changePart={props.changePart} changeLanguage={props.changeLanguage} updateFollowUpScoreMatrix={props.updateFollowUpScoreMatrix} updateFollowUpAnswerSummary={props.updateFollowUpAnswerSummary} />)
    }
  }

  return(
    <Questionnaire partID="10-1" questionFields={questionFields} getResult={getResult} currentLanguage={props.state.currentLanguage} stateMain={props.stateMain} data={props.data} restartTestMain={props.restartTestMain} />
  )
}